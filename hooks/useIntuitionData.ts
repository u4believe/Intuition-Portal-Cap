...
export function useAllClaims(page: number = 1, search: string = '') {
  return useQuery({
    queryKey: ['allClaims-v4', page, search],
    queryFn: async () => {
...
export function useTriples(page: number = 1, search: string = '') {
  return useQuery({
    queryKey: ['triples-v3', page, search],
    queryFn: async () => {
      const params = new URLSearchParams({ page: String(page) })
      if (search) params.set('search', search)
    ...
    queryFn: async () => {
      ...
      return {
        atoms: (data.atoms || []).filter((atom: any) => !isUnknownEntry(atom)).map((atom: any) => ({
          ...
          creatorId: atom.creatorId || '',
          creatorLabel: atom.creatorLabel || '',
          creatorImage: atom.creatorImage || null,
          subjectLabel: '',
          subjectType: '',
          predicateLabel: '',
          predicateType: '',
